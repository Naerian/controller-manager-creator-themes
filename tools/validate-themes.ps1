...
function Appearance($theme, $folder, $file, $rules, $fontIds) {
    ...
    foreach ($p in $doc.PSObject.Properties) {
        ...
        switch ($r.Kind) {
            ...
            'order' { $parts=if ($v -is [string]) {@($v.Split(',')|%{$_.Trim()})} else {@()}; $withoutTimer=@($parts|?{$_-ne'Timer'});$classic=@('Title','Controller','Metadata','Instruction','Status');$alert=@('Incident','Title','ControllerName','Metadata','Instruction','Status');$valid=(@($parts|sort -Unique).Count-eq$parts.Count)-and((($withoutTimer.Count-eq$classic.Count)-and-not@($withoutTimer|?{$classic-notcontains$_}).Count)-or(($withoutTimer.Count-eq$alert.Count)-and-not@($withoutTimer|?{$alert-notcontains$_}).Count));if(-not$valid){Fail $theme "$($p.Name) must contain every classic or alert-layout block exactly once; Timer is optional"} }
        }
    }
    return $doc
}
...
$od=@(
 ...
 @('ShowTitle','bool'),@('UppercaseTitle','bool'),@('ShowInstruction','bool'),@('ShowPauseStatus','bool'),@('ShowDisconnectTimer','bool'),@('ShowControllerName','bool'),@('ShowControllerIcon','bool'),@('ShowStatusIcon','bool'),@('ShowConnectionBadge','bool'),@('ShowBatteryBadge','bool'),@('ControllerIconPosition','enum',$null,$null,@('Left','Center','Right','Top')),@('ControllerIconSize','int',16,96),@('StatusIconSize','int',12,64),
...
Get-ChildItem $themeRoot -Directory | % {
    ...
    $fontIds=@{};foreach($f in @($m.Fonts)){
        ...
        if([IO.Path]::IsPathRooted($ff)){Fail $theme "font '$($f.Id)' Folder must be relative"}else{try{$fontRoot=[IO.Path]::GetFullPath($folder.FullName).TrimEnd('\')+'\';$fp=[IO.Path]::GetFullPath((Join-Path $folder.FullName $ff));if(-not$fp.StartsWith($fontRoot,[StringComparison]::OrdinalIgnoreCase)-or-not(Test-Path $fp)){Fail $theme "font '$($f.Id)' Folder is missing or unsafe"}elseif(-not@(Get-ChildItem $fp -File|?{$fonts-contains$_.Extension.ToLowerInvariant()}).Count){Fail $theme "font '$($f.Id)' Folder contains no supported fonts"}}catch{Fail $theme "font '$($f.Id)' Folder is invalid"}}
    }
