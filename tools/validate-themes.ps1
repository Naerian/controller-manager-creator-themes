...
function Appearance($theme, $folder, $file, $rules, $fontIds) {
    ...
    foreach ($p in $doc.PSObject.Properties) {
        ...
        switch ($r.Kind) {
            ...
            'order' { $parts=if ($v -is [string]) {@($v.Split(',')|%{$_.Trim()})} else {@()}; $expected=@('Title','Controller','Metadata','Instruction','Status'); if ($parts.Count -ne 5 -or @($parts|sort -Unique).Count -ne 5 -or @($parts|?{$expected -notcontains $_}).Count) { Fail $theme "$($p.Name) must contain each documented block exactly once" } }
        }
    }
    return $doc
}
...
$nd=@(
 ...
 @('ShowIconContainer','bool'),@('IconContainerColor','color'),@('IconContainerBorderColor','color'),@('IconContainerBorderThickness','int',0,8),@('IconContainerCornerRadius','int',0,40),@('IconContainerPadding','int',0,24),@('ShowConnectionBadge','bool'),@('BadgePosition','enum',$null,$null,@('Content','Icon','Bottom')),
...
$or=@{}
$od=@(
 @('ScalePercent','int',80,140),@('CardWidth','int',320,1000),@('CardPosition','enum',$null,$null,@('Center','Top','Bottom','TopLeft','TopRight','BottomLeft','BottomRight')),@('ScreenMargin','int',0,160),@('LayoutMode','enum',$null,$null,@('Standard','Split','Hero')),@('ContentAlignment','enum',$null,$null,@('Left','Center','Right')),@('Animation','enum',$null,$null,@('None','Fade','FadeScale','Slide')),@('BlockOrder','order'),@('MetadataOrientation','enum',$null,$null,@('Horizontal','Vertical')),@('Padding','int',12,80),@('ElementSpacing','int',0,48),
 ...
 @('DimColor','color'),@('CardColor','color'),@('UseGradient','bool'),@('GradientColor','color'),@('GradientAngle','int',0,359),@('AccentColor','color'),@('TextColor','color'),@('WarningColor','color'),@('UseBackgroundImage','bool'),@('BackgroundImagePath','image'),@('BackgroundImageStretch','enum',$null,$null,@('UniformToFill','Uniform','Fill')),@('BackgroundImageHorizontalAlignment','enum',$null,$null,@('Left','Center','Right')),@('BackgroundImageVerticalAlignment','enum',$null,$null,@('Top','Center','Bottom')),@('BackgroundImageOpacity','int',0,100),@('BackgroundImageTintOpacity','int',0,100),
...
Get-ChildItem $themeRoot -Directory | % {
    ...
    if($null-ne$o){if($o.OverlayUseBackgroundImage-eq$true-and[string]::IsNullOrWhiteSpace($o.OverlayBackgroundImagePath)){Fail $theme 'enabled overlay image requires BackgroundImagePath'};if(-not(Test-Path (Join-Path (Join-Path $Root "previews\$theme") 'overlay.png'))){Fail $theme 'missing preview overlay.png'}}
